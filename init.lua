...
local function selfControl()
	incrementTime()
	local output = hs.execute("/Applications/SelfControl.app/Contents/MacOS/selfcontrol-cli is-running 2>&1")
	...
	startSelfControl = function()
		local selfcontrol_task = hs.task.new(selfcontrol_command, selfcontrol_callback, selfcontrol_arguments)
		...
		hs.timer.doAfter(5, function()
			if selfcontrol_task:isRunning() then
				print("User isn't responding, reprompting...")
				selfcontrol_task:terminate()
				startSelfControl()
			end
		end)
	end
